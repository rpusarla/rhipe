...
{    
    ...
    {        
        ...
        private const string RegExPattern = @"(DRAW A(N)? (ISOSCELES|EQUILATERAL|SCALENE) TRIANGLE)|(?:\S* \w+ \d+)";
        ...
        public Token ParseData(string inputText)
        {
            ...
            {                
                ...
                else
                {
                    ...
                    // Scalene Triangle
                    if (objTokensViewModel.TriangleName == Constants.ScaleneTriangle)
                    {
                    ...
                    // Isosceles Triangle
                    if (objTokensViewModel.TriangleName == Constants.IsoscelesTriangle)
                    {
                        if (matches.Count(match => match.Value.IndexOf(BaseOfConst, StringComparison.Ordinal) != -1) == 1 &&
                    ...
                    if (objTokensViewModel.TriangleName == Constants.EquilateralTriangle)
                    {
